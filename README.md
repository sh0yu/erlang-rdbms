...
Erlangで書いたリレーショナルデータベース。SQLのパーサ・意味解析・プランナ・
実行器、ページ単位のディスクストレージ、バッファプール、B+tree索引、
WALによるクラッシュリカバリ、スナップショット分離による並行制御を備えている。

手書き 11,748行 + テスト 7,990行。OTPアプリケーション名は `transaction_db`。

正しさは第三者のテストスイートで確かめてある。

| | |
| --- | --- |
| [sqllogictest](https://www.sqlite.org/sqllogictest/)(SQLiteのコーパス) | 10,706レコード中 **10,700 pass / 0 wrong** |
| [Hermitage](https://github.com/ept/hermitage)(分離水準) | 15本中 **14本が PostgreSQL と一致**(残り1本はこちらが強い側) |
| 自前 | 525 tests、dialyzer clean |

**何ができて何ができていないかは、下の「制限」節に全部書いてある。**
「作ったつもりで間違っている」状態を避けることを、この実装の第一の基準にした。
`ORDER BY 1` が黙って無視される、結合条件が1つも見えていない、といった
**エラーにならない種類のバグ**を、上のスイートで見つけて潰した経緯は
`STAGE7.md` に記録してある。

ブラウザUI(`bin/sqlweb`)は、1文流すたびにその文の実行計画とエンジンの内部状態を
並べて表示する。索引を張ると計画が変わる、別の接続が別の版を見ている、といった
動きが画面で確認できる。
...
```sh
cd erlang-rdbms
bin/sqlweb      # ブラウザUI(教材) http://127.0.0.1:8080
bin/sqlsh       # 端末クライアント
```
...
SQLが動くだけの画面では、DBの中で何が起きているかは分からない。
**1文流すたびに、その文の実行計画とエンジンの状態を一緒に出す。**

```
┌──────────────┬──────────────────────────┬──────────────────────────┐
│ 台本         │ SQL と結果               │ 中で何が起きているか     │
│              │                          │                          │
│ 1. まず動かす│ @A SELECT * FROM fruit   │ 実行計画                 │
│ 2. プランナ  │    WHERE name = 'banana' │   Project (id, name)     │
│ 3. 結合      │                          │     Index Scan on fruit  │
│ 4. NULL      │  id │ name   │ price     │                          │
│ 5. トランザ… │   2 │ banana │ 150       │ 接続 A  トランザクション中│
│ 6. 列制約    │                          │   分離水準 READ ONLY     │
│ 7. スナップ… │                          │   見ている版 7           │
│ 8. 分離水準  │                          │ 接続 B  アイドル         │
│ 9. READ COM… │                          │                          │
│ 10. 行ロック │                          │ undo 3版 / スナップ1     │
│ 11. 集約…    │                          │ 待ち: なし               │
└──────────────┴──────────────────────────┴──────────────────────────┘
```

**左の台本**は `priv/demos.sql`。11本のデモが「1文ずつ」並んでいて、
`▶ 次の1文` を押すと次に進む。何を見せる文なのかが手順ごとに書いてある。
台本はコードに埋め込まず外に置いてあるので、書き足すのに再コンパイルは要らない。

**右の実行計画**は、その文を流す**前**にプランナへ通した結果。索引を作ると
`Seq Scan` が `Index Scan` に変わるところがそのまま見える。`UPDATE` /
`DELETE` はプランナを通らないので、その旨を書く(黙って空欄にしない)。

**右のエンジンの状態**は、各接続がどの版を見ているか、undo が何版残って
いるか、誰が何を待っているか、プランナがカタログで何を見ているか。
並行制御はここが見えないと理解のしようがない。

**接続は A と B の2本**を持つ。並行制御は1本では見せられない。片方で
`BEGIN` したまま同じ行を書くと、もう片方が待たされる/断られるのが見える。
デモ7〜10がそれで、`@A` / `@B` がどちらで流すかを表す。

台本は試験でも流している(`test/web_ui_demo_tests.erl`)。動かない手順が
あると説明ごと嘘になるので、全手順をそのまま実行して確かめている。
`!error` と書いた手順は**断られるのが正しい**もので、それも期待値として扱う。
...
`BEGIN` していない文は、**その1文だけのトランザクション**で走る(自動コミット)。
SQL では文はかならずトランザクションの中で走り、明示していなければ
「その文だけ」になる、というのが規格の決まり。`COMMIT` / `ROLLBACK` だけは
対象外で、開いていないものは閉じられない。
...
| pass | **10,700** |
| wrong(実行できたが答えが違う) | **0** |
| unsupported(構文・機能が無い) | 6 |

見るのは wrong が0であること。unsupported は「まだ作っていない」なので
落とさない。落とすべきなのは「作ったつもりで間違っている」ほう。

残る6件は複合索引(5)と索引の列の向き(1)。どちらも索引の作り方の話で、
結果の正しさには関わらない。

##### 流して見つかったもの

通過率は 400 → 10,700 になった。途中で見つかったものを順に。

| | 見つかったもの |
| --- | --- |
| **`ORDER BY 1` が黙って無視される** | 裸の整数は出力列の位置を指す(SQLの規則)のに、定数式として扱っていた。定数で並べても順序は決まらないので、**並べ替えなしになる**。誤答180件はすべてこれ1つが原因 |
| **結合条件が1つも見えていなかった** | `FROM t1, t2 WHERE a1 = b2` のように表名を書かない書き方で、`pred_keys/1` が「どの表か分からない」として捨てていた。つながりが見えないと貪欲法は素直に**直積**を選ぶ。表が増えるほど爆発し、select5(最大62表の結合)が終わらなくなっていた。カタログを引けばどの表の列かは分かる |
...
| `PRIMARY KEY` / `UNIQUE` / `NOT NULL` が無い | 64個の `CREATE TABLE` が倒れ、1,372文が巻き添え |
...
**一意性の検査だけは分離水準の外にある。** 一意性は「いまコミットされて
いる全体」に対する条件であって、自分が見ている版に対する条件ではない。
自分のスナップショットで探すと、自分が始めた後に入った行を見落として
重複を通す。よってここだけ共有データを直に引く。PostgreSQL も同じ。

並行して同じ鍵を入れる2本は、双方とも「まだ無い」と判断してしまう。
どちらの行も相手の未コミット領域にあって、共有データに現れていないため。
...
これは行ではないので、共有データに実体が無くてもロックできる。先に取った
方が進み、後から来た方は待つ。ロックが解放されるのはコミットの**適用が
終わった後**なので、待っていた方が共有データを引いた時点では相手の行が
見えている。

穴を1つ。衝突相手の行を別の接続が削除しようとしていて、まだコミットして
いない場合、こちらはその行を「ある」と見て断る。PostgreSQL は相手の決着を
待つ。待たずに断るぶん厳しい側に外れる。
...
- `BETWEEN` の両辺には論理演算子を書けない(`x BETWEEN (a AND b) AND c` は
  括弧が要る)。SQLの規格も同じ制限
- 列制約は `PRIMARY KEY` / `UNIQUE` / `NOT NULL` のみ。表レベルの
  `PRIMARY KEY (a, b)`、`CHECK`、`FOREIGN KEY`、`DEFAULT` は未実装
- 一意性の検査は、衝突相手を別の接続が削除中(未コミット)のとき待たずに断る
