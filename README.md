...
代わりに `snapshot_mng` からスナップショットを1つ取り、
トランザクションの間じゅう持つ。**読み手と書き手は互いを待たない。**
...
順序が要点で、**undo を積む → 適用する → 見える版を上げる** の順に進む。
...
**直列化可能性は保たれる。** スナップショット分離の異常(write skew など)は
書き込みを伴って初めて起きる。読み取り専用なら、そのスナップショットの版と
同じ位置に並べられる。
...
**索引はスナップショットの読み手には見せない。** 索引は現在の値で引かれる。
ある行の索引付き列が更新されると、その行は古い鍵では引けなくなる。
undo は行の中身しか戻せず、索引そのものを版管理していないので直せない。
よって読み取り専用では全表走査を選ぶ(`EXPLAIN` で確認できる)。

DDL とだけは排他のままで、`commit_latch` がそれを担う。読み手は走査の間だけ
共有ラッチを持ち、DDL は排他ラッチを取る。走査中に表が落ちるのを防ぐ。
...
**段階1・段階2は実装済み**(`BEGIN READ ONLY` とスナップショット分離)。
読み取り専用トランザクションは直列化の列に並ばず、互いにも書き手とも
待ち合わせない。8接続の読み取りで 94.4ms → 14.1ms。
...
**パーティショニングか読み手の分離と組み合わせたとき**で、本実装は後者を持った。

段階:

1. ~~**単一書き手 + 並行読み手**~~ — 実装済み。`commit_latch` が
   コミットの適用を囲い、読み手はその間だけ止まった。
   ただし長い照会がコミットを止めるという弱点があった
2. ~~**スナップショット分離**~~ — 実装済み。`snapshot_mng` が変更前の値を
   保持し、読み手は開始時点へ巻き戻して読む。コミットは読み手を待たない。
   追記専用ストレージやバージョン鎖ではなく、MySQL/InnoDB と同じ
   **undo による巻き戻し**を選んだ。既存の行を書き換える形を変えずに済み、
   ストレージ層に手を入れなくてよいため。代償は
   「古い読み手は undo の上限を超えると断られる」と
   「スナップショットの読み手は索引を使えない」の2点
3. **書き手の並行化**。ここで初めて `lock_mng` が仕事をする。
   同時に、デッドロック検出・走査のロック粒度(ファントム)・
   write skew が問題になる

書き手を並行にすると、いまの「読み取り専用は必ず直列化可能」は成り立たなくなる。
スナップショット分離のままでは write skew が入るので、
SSI(直列化可能スナップショット分離)か述語ロックが要る。
...
- 分離レベルは、書き手は直列実行、読み手はスナップショット分離。
  行ごとのバージョン鎖(xmin/xmax)を持つ本来のMVCCではなく、
  コミット単位の undo をメモリに積んで巻き戻す方式
- スナップショットの読み手は索引を使えない(索引が版を持たないため)。
  必ず全表走査になる
