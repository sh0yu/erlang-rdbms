...
| `sys_tbl_mng` | システムカタログ(DETSに永続化)。読みはETSキャッシュから直接 |
| `simple_index` | ETSハッシュによるカラムインデックス(既定) |
| `index` | B+treeによるカラムインデックス。範囲検索ができる |
| `data_buffer` | バッファプール。走査は呼び出し元がETSを直接読む |
...
よって「自分の視界の外のコミットが、自分の書く行を変えていないか」を
確かめる。変わっていたら自分を捨てる(first-updater-wins、
`serialization_failure`)。

**確かめるのは `UPDATE` / `DELETE` の時点**で、コミットまで待たない。
待つと、残りの文を全部やってから捨てることになる。PostgreSQL が
`could not serialize access due to concurrent update` を UPDATE の時点で
返すのと同じ位置。コミット時にももう一度確かめる(挿入経路のため)。
...
分離の仕組みが揃っても、**全接続が同じ1プロセスの前に並んでいれば**
並行にはならない。実際そうなっていた。32接続が同時に走査したときの
メッセージキュー長を測ると、こうなっていた。

| | `data_buffer` | `sys_tbl_mng` | `snapshot_mng` | `lock_mng` |
| --- | ---: | ---: | ---: | ---: |
| 修正前 | 32 | 31 | 2 | 0 |
| 修正後 | 0 | 0 | 6 | 0 |

キュー長が接続数と一致する = 全員が待っている、ということ。詰まっていたのは
並行制御ではなく、その下の2つだった。どちらも「gen_server がデータを所有し、
全アクセスがそこを通る」形になっていた。

**カタログ(`sys_tbl_mng`)** — 読みが極端に多く、書きは DDL のときだけ。
`read_data_oid/2` は行ごとに `exist_table/2` を呼ぶので、索引で100行引けば
往復が100回。DETS の内容を ETS に写しておき、**読みは呼び出し元が直接引く**。
gen_server が受け持つのは書き込みだけで、「DETS を書く → ETS に写す」の順に進む。

**バッファプール(`data_buffer`)** — ページを1枚読むたびに `gen_server:call`
だった。フレームは元から public な ETS なので、載っているページを読むだけなら
呼び出し元が直接引ける。そのために索引を1つ置く。

```
buf_dir : {TableName, PageId} -> {BufName, Gen}

読み手: 1. buf_dir を引く   2. フレームから行を読む
        3. buf_dir をもう一度引き、同じ {BufName, Gen} かを確かめる
        違っていれば追い出されたので gen_server へ回す
```

`Gen`(世代)が要るのは A→B→A と入れ替わった場合を捕まえるため。
追い出す側は「索引から消す → フレームを空にする」の順に進む。逆にすると、
空になったフレームを読み手が有効と見なす。

フレーム数も 8 固定から既定 256(`buffer_frames` で変更可)にした。
走査が gen_server の中で直列に走っていた頃は少なくても目立たなかったが、
並列に走らせると取り合いになる。

実測(3000行の表を N 接続が同時に全走査):

| 接続数 | 修正前 | 修正後 |
| --- | --- | --- |
| 1 | 172 scans/s | 556 scans/s |
| 8 | 615 scans/s | 4444 scans/s |
| 32 | 727 scans/s(頭打ち) | **6957 scans/s** |
...
#### SQL の検証(sqllogictest)

[sqllogictest](https://www.sqlite.org/sqllogictest/) は SQLite が
「同じSQLを複数のDBに流して結果が一致するか」を確かめるために作った形式。
クエリと期待結果が組で書いてあり、結果が多いときは md5 に畳んである。
DuckDB や CockroachDB もこれを使っている。

`test/slt.erl` がランナー。コーパス(数MB)は他所のものなので repo には
入れていない。`bin/slt-fetch` で落とすと `rebar3 eunit` が拾って流す。

| | select1〜5 の10,706レコード |
| --- | --- |
| pass | **9,264** |
| wrong(実行できたが答えが違う) | **0** |
| unsupported(構文・機能が無い) | 1,442 |

見るのは wrong が0であること。unsupported は「まだ作っていない」なので
落とさない。落とすべきなのは「作ったつもりで間違っている」ほう。

残る 1,442 の内訳は3つだけ。

| 件数 | 何が無いか |
| ---: | --- |
| 1,436 | `PRIMARY KEY` 列制約(64個の `CREATE TABLE` が倒れ、その表への1,372文が巻き添え) |
| 5 | 複合索引 `CREATE INDEX ... ON t(a, b, c)` |
| 1 | 索引の列の向き `CREATE INDEX ... ON t(a DESC)` |

`PRIMARY KEY` は構文だけ受け取って一意性を確かめない、という実装はしない。
それは「作ったつもりで間違っている」側になる。一意性を並行実行の下で
正しく確かめるには、自分のスナップショットに映らない他の接続の未コミットの
挿入まで見る必要があり、いまの作りには無い。

##### 流して見つかったもの

コーパスを流して**3つの実バグと2つの欠落**が出た。最初の通過率は
400 pass / 180 wrong だった。

| | |
| --- | --- |
| **`ORDER BY 1` が黙って無視される** | 裸の整数は出力列の位置を指す(SQLの規則)のに、定数式として扱っていた。定数で並べても順序は決まらないので、**並べ替えなしになる**。誤答180件はすべてこれ1つが原因 |
| 簡易 `CASE x WHEN v THEN ...` が無い | 探索 `CASE WHEN cond THEN ...` しか無かった。197レコード |
| `BETWEEN` が無い | 254レコード。下記 |
| `VARCHAR(30)` の長さ指定が通らない | `CREATE TABLE` ごと落ちて、その表への3,842文が巻き添えになっていた |

`BETWEEN` は以前「yecc では書けない」として見送っていたが、書けた。
詰まっていたのは優先順位ではなく、**両辺に `AND` を含む式を許していた**こと。
`x BETWEEN 1 AND 2` を読んだ時点で、その `AND` が BETWEEN のものか論理演算子か
決められず、reduce/reduce が38件出る。SQLの規格も BETWEEN の両辺を
論理演算子を含まない式に限っているので、そのとおりに専用の非終端記号
(`b_expr`)を作ったら競合はゼロになった。括弧で囲めば中はフル式に戻せる。

#### 分離水準の検証(Hermitage)

「スナップショット分離である」と自分で言うだけでは、自分で決めた定義を
自分で確かめているにすぎない。[Hermitage](https://github.com/ept/hermitage)
は Martin Kleppmann が各DBを同じ手順で並べるために書いたもので、
異常ごとに **どの文をどの順で流すか** が決まっている。その PostgreSQL 版の
手順をそのまま写して `test/hermitage_tests.erl` に置いた。

| | 内容 | `READ COMMITTED` | `BEGIN`(既定) |
| --- | --- | --- | --- |
| G0 | dirty write | 防ぐ | 防ぐ |
| G1a | 捨てられた書き込みを読む | 防ぐ | 防ぐ |
| G1b | 途中の値を読む | 防ぐ | 防ぐ |
| G1c | 互いの未コミットを読む | 防ぐ | 防ぐ |
| OTV | コミットの片側だけが見える | 防ぐ | 防ぐ |
| PMP(読み) | 後から入った行が見える | 防がない | 防ぐ |
| PMP(書き) | 消したはずの行が残る | **防ぐ**(下記) | 防ぐ |
| P4 | lost update | 防がない | 防ぐ |
| G-single | read skew | 防がない | 防ぐ |
| G2-item | write skew | 防がない | **防がない** |
| G2 | anti-dependency cycle | 防がない | **防がない** |

15本のうち14本が PostgreSQL と同じ結果になった。ずれたのは
**PMP(書きの述語)の READ COMMITTED** の1本だけで、しかも
こちらのほうが強い側にずれている。

違いは「ロックを待たされた後に何をやり直すか」から来ている。

| | やり直すもの |
| --- | --- |
| PostgreSQL | 待っていた**その行だけ**を読み直して条件を当て直す(EvalPlanQual) |
| このDB | **文を頭からやり直す**。走査もやり直す |

初期値 `1=>10, 2=>20` に T1 が全行 +10 して `1=>20, 2=>30` にする。
T2 の `DELETE ... WHERE value = 20` は走査した時点で id=2 に当たり、そこで止まる。

* PostgreSQL は id=2 だけを読み直す。30 なので条件に合わず何も消さない。
  走査し直さないので、新たに 20 になった id=1 は最後まで見ない。
  結果、消したはずの 20 が残る
* このDBは走査からやり直し、`1=>20, 2=>30` を見て id=1 を消す

こちらの結果は「T1 の後に T2 を丸ごと実行した」場合と一致する。
丸ごとやり直せるのは、行ロックを取るのがローカル領域に書く**前**なので、
やり直しても捨てるものが無いため。ただしトランザクション全体が
直列化可能になるわけではない。同じトランザクションの前の文は、
古いスナップショットで書いたままやり直されない。

最下段の2つ(G2-item / G2)は既定でも防げない。これがスナップショット分離の
限界で、防ぐには SSI か述語ロックが要る。
...
| `buffer_frames` | `256` | バッファプールのフレーム数。1フレーム = 1ページ |
...
- 入れ子が深いとき、内側が使う外側の参照を見つけると外側も
  ...
  畳まないことがある
- ウィンドウ関数は未実装
- `BETWEEN` の両辺には論理演算子を書けない(`x BETWEEN (a AND b) AND c` は
  括弧が要る)。SQLの規格も同じ制限
- `PRIMARY KEY` / `UNIQUE` / `NOT NULL` の列制約は未実装。構文だけ受けて
  確かめない実装はしていない
- 索引は単一カラムのみ。複合索引・索引の列の向きは未実装
- `VARCHAR(n)` の長さは受け取るが**使わない**。切り詰めも拒否もしない
- `CAST` は未実装
